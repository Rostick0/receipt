...
<div class="receipt-item">
    <div class="receipt-item__top">
        <a class="receipt-item__id text-blue" href="{{ route('receipt.show', ['receipt' => $receipt->id]) }}">
            Чек № {{ $receipt->id }}
        </a>
        <svg class="receipt-item__star @if ($receipt->folder_receipts->where('receipt_id', $receipt->id)->count()) _active @endif @isset($classStar){{ $classStar }}@endisset"
            data-id="{{ $receipt->id }}"
            @isset($folderReceiptId)data-folder-receipt-id="{{ $folderReceiptId }}"@endisset width="16"
            ...
            <path
                ...
                stroke="rgb(255, 160, 0)" fill="transparent" stroke-width="2" stroke-linecap="round"
    ...
    <div class="receipt-item-info">
        ...
        <div class="receipt-item-info__total">
            ...
            <div class="receipt-item-info__hr"></div>
            @isset($comment)
                <form action="{{ route('folder-receipt.update', ['folder_receipt' => $folderReceiptId]) }}" method="POST">
                    @csrf
